...
(function () {
  ...
  const COMBOS = {
    ...
    source_control_mutation_path: {
      ...
      sev: "high",
      derived: "A tracked file was written and a reachable push credential (a readable SSH key, gh/credential-store token, or live ssh-agent) can authenticate the push.",
      legs: ["git.push_likelihood"],
      nodes: ["git.push_likelihood"],
      evidence: ["file-write to a tracked file (event trigger)", "+ push likelihood: likely (reachable push credential)", "→ commit + push composes"],
    },
    high_review_risk: {
